...
func TestRegistry(t *testing.T) {
	...
	statusLogger := NewStatutsLogger()

	nodeRegistry := NewNodeRegistry(nodeConfig, statusLogger)

	// test register function
	nodeInfo := &NodeInfo{IPAdrees: "929290", PortNumber: 6349}
	...
	if receivedNode.IPAdrees != nodeInfo.IPAdrees || receivedNode.PortNumber != nodeInfo.PortNumber {
		t.Errorf("node list is not correct; ecpected node %v, reveived node %v \n", nodeInfo, receivedNode)
	}

	nodeRegistry.Unregister(fmt.Sprintf("%s:%d", nodeInfo.IPAdrees, nodeInfo.PortNumber))
	nodeList = &NodeList{}
	err = nodeRegistry.GetNodeList(nodeInfo, nodeList)
	if err != nil {
		t.Error(err)
